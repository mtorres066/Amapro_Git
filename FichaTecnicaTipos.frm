...
Begin VB.Form FichaTecnicaTipos 
   ...
   ClientTop       =   330
   ClientWidth     =   8415
   Icon            =   "FichaTecnicaTipos.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5175
   ...
   Begin TabDlg.SSTab TabPuestos 
      ...
      TabPicture(0)   =   "FichaTecnicaTipos.frx":2A12
   ...
   End
End
Attribute VB_Name = "FichaTecnicaTipos"
...
Private Sub CmdBuscar_Click(Index As Integer)
        ...
        If Index = 0 Then
            If OptCodigo.Value = True Then
                If GOrigenDeDatos = "AmaproAccess" Then
                    Call Abrir_Recordset(RDocumentos, "Select * from FichaTecnicaTipos where CodigoTipo Like '%" & TxtBuscar.Text & "%'")
                Else 'ORACLE
                    Call Abrir_Recordset(RDocumentos, "Select * from FichaTecnicaTipos where UPPER(CodigoTipo) Like '%" & UCase(TxtBuscar.Text) & "%'")
                End If
            ElseIf OptDescripcion.Value = True Then
                If GOrigenDeDatos = "AmaproAccess" Then
                    Call Abrir_Recordset(RDocumentos, "Select * from FichaTecnicaTipos where Descripcion Like '%" & TxtBuscar.Text & "%'")
                Else 'ORACLE
                    Call Abrir_Recordset(RDocumentos, "Select * from FichaTecnicaTipos where UPPER(Descripcion) Like '%" & UCase(TxtBuscar.Text) & "%'")
                End If
            End If
        'SELECCIONAR TODOS LOS DATOS
        ElseIf Index = 1 Then
                Call Abrir_Recordset(RDocumentos, "Select * From FichaTecnicaTipos")
        End If
